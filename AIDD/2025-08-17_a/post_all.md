...
date: "2024-08-17"
categories: ["AI 药物发现", "计算生物学", "药物设计", "分子对接"]
...
## 目录  
1. CellForge 不只是一个预测工具，它是一条能自动将生物学问题转化为高效计算模型的「流水线」，有望将科学家从繁琐的建模工作中解放出来。
2. AI 模型给蛋白质口袋画了一张「倾向图」，告诉药物化学家把不同的原子放在哪里才能获得最佳结合。
3. 这个模型终于开始正视药物发现中那个被长期忽视的「房间里的大象」——溶剂效应，强迫 AI 承认分子是活在「水」里，而不是真空中。
...
这个模型目前有个巨大的短板：它对化学键「一无所知」。它能告诉你「在这里放个碳，旁边放个氮」，但它不知道你是否能用现实中的化学反应，稳定地把这两个原子连在一起。这就像一个建筑 AI，设计了一座无比酷炫的悬浮城堡，却完全没考虑过重力和材料强度。最后，还是得满身化学试剂味道的「包工头」，去想办法把图纸变成现实。  
