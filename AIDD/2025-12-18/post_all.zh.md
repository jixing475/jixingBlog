---
title: "CryoBoltz: 用 AI 透视蛋白质动态，解锁药物新靶点"
description: |
  传统蛋白质结构预测如同拍摄静态'证件照'，但药物研发更需了解其动态'录像'。CryoBoltz 技术创新地利用模糊的冷冻电镜 (cryo-EM) 数据，指导 AI 模型解析蛋白质的多种动态构象。通过'多尺度引导'机制，该方法能从包含多种状态的'虚影'数据中，重构出蛋白质在功能活动中的一系列清晰结构，从而为靶向特定构象的药物设计解锁新的药物靶点，提升研发效率。
date: "2025-12-18"
categories: ["CryoBoltz", "冷冻电镜", "蛋白质动态构象", "扩散模型", "药物发现"]
image: "post_all_image/multiscale_guidance_2025_12_11_image_1.jpg"
image-alt: |
  一张科学概念图，展示了 CryoBoltz 方法的核心原理。图像中央是一条清晰，多彩的蛋白质三维结构模型，呈螺旋和折叠的带状形态。该模型被一团半透明，泛着蓝光的模糊云雾所包围，这团云雾代表了从冷冻电镜实验中获得的异质性密度图数据，它包含了蛋白质多种动态构象的混合信息。数条发光的箭头从模糊的云雾指向清晰的蛋白质模型，形象地说明了 CryoBoltz 利用模糊实验数据作为'向导', 引导 AI 模型生成符合该数据约束的精确蛋白质结构。这张图揭示了如何将'模糊'的实验快照转化为对蛋白质动态过程的清晰洞察，这对于发现与特定功能状态相关的药物靶点至关重要。
toc-depth: 3
---
